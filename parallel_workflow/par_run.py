...
def compute_pairs():
    ...
    output_file_name = f"par_pairs_{os.path.splitext(selected_file_name)[0]}.json"
...
def compute_cumulants():
    ...
    if num_dimensions == 2:
        ...
        output_cumulant_file_name = f"par_cum_3rd_{os.path.splitext(selected_data_file_name)[0]}.csv"
    elif num_dimensions == 3:
        print("Computing 4th-order cumulant...")
        cumulant_result = compute_4th_order_cumulant(df_associated, num_chunks)
        output_cumulant_file_name = f"par_cum_4th_{os.path.splitext(selected_data_file_name)[0]}.csv"
    else:
